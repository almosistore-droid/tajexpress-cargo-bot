...
# 🚨 ИСПРАВЛЕНО: Теперь используется ваш реальный URL с Render.
WEBHOOK_HOST = 'https://tajexpress-cargo-bot.onrender.com' # <--- ИСПРАВЛЕНО
WEBHOOK_ROUTE = '/' + TOKEN
...
# --- 3. КРИТИЧЕСКИЙ ВЫЗОВ WEBHOOK (ВЫПОЛНЯЕТСЯ ПРИ ИМПОРТЕ GUNICORN) ---
set_webhook()


# --- 4. КОНФИГУРАЦИЯ FLASK/WEBHOOK ---
...
if __name__ == '__main__':
    # Эта часть выполняется только при прямом запуске через `python cargo_bot.py`
    print("APPLICATION LOG: Приложение запущено в локальном режиме.")
